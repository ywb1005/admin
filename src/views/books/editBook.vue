<template>
    <div class="container">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>修改书籍</el-breadcrumb-item>
      </el-breadcrumb>
      <hr>
      <el-row style="margin-top:40px;width: 600px;">
        <el-form :model="form" ref="form" :rules="rules" label-width="120px">
          ...
          <el-form-item class="user-form">
            <el-button type="primary" @click="onSubmit">保 存 修 改</el-button>
          </el-form-item>
        </el-form>
      </el-row>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getBookInfo (id) {
        this.$http.get(API + 'book/info/' + id, {
          headers: {'Authorization': 'Bearer ' + user.token}
        }).then((response) => {
      ...
      onSubmit () {
        let id = this.$route.params.id
        this.$http.post(API + 'book/edit/' + id, this.form, {
          headers: {'Authorization': 'Bearer ' + user.token}
        }).then((response) => {
          if (response.data.code === 0) {
            this.$message({
