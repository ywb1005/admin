...
<script>
  ...
  export default {
    ...
    methods: {
      getSectionInfo (id) {
        this.$http.get(API + 'book/section/info/' + id, {
          headers: {'Authorization': 'Bearer ' + user.token}
        }).then((response) => {
          if (response.data.code === 0) {
            this.form.name = response.data.data.nodeName
            this.form.index = response.data.data.nodeId
            this.imageUrl = this.form.image = response.data.data.nodeImg
          }
        }, (response) => {
          this.$message({
      ...
      onSubmit () {
        ...
        let id = this.$route.params.sectionId
        this.$http.post(API + 'book/' + bookId + '/section/edit/' + id, this.form, {
